...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!empty($_POST["matricul"]) && !empty($_POST["mot_de_passe"])) {
        try {
            $query = "SELECT * FROM utilisateur WHERE matricul = :matricul";
        ...
        if ($resultat) {
            if (password_verify($_POST["mot_de_passe"], $resultat['mot_de_passe'])) {
                ...
                if ($resultat["type_utilisateur"] == 'surveillants') {
                    header("location:opp.php");
                //} elseif ($resultat["type_utilisateur"] == 'surveillants') {
                  //  header("location:opp.php");
               // } else {
